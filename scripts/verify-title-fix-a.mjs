/**
 * 方案 A 真机验证：真实 shim + 真凭据，模拟 DSH 标题请求（max_tokens=64）。
 * 断言：shim 识别标题请求并把预算提到 1024（warnings 可见），
 * 且在 1024 预算下各模型正常输出正文（mimo 系留给方案 B 的 reasoning off）。
 */
...
for (const model of MODELS) {
  ...
  const ok = text.length > 0 && raised
  if (!ok) failures++
  console.log(
    `${ok ? 'OK  ' : 'FAIL'} ${name.padEnd(18)} text=${JSON.stringify(text).slice(0, 26).padEnd(28)}`
    + ` raised=${raised} ${Date.now() - t0}ms`,
  )
  if (!raised) console.log(`      warnings: ${warnings.join(' | ') || '(none)'}`)
}
