#!/usr/bin/env node
import { S as defaultComateHome, h as safeMessage, o as runComateCheck, q as isSealedComateSecret, s as ComateUpstreamClient, t as COMATE_CONNECT_VERSION, x as ComateCredentialStore } from "./version-BJtfo7Fd.js";
...
async function doctor(jsonOutput) {
	const report = await makeStore().doctor(COMATE_CONNECT_VERSION);
	if (jsonOutput) printJson(report);
		...
		...report.candidates.map((candidate) => `  - ${candidate.path}\n    present=${candidate.present} valid=${candidate.valid}${candidate.baseUrl === void 0 ? "" : ` baseUrl=${candidate.baseUrl}`}${candidate.modelCount === void 0 ? "" : ` models=${candidate.modelCount}`}${candidate.error === void 0 ? "" : ` (${candidate.error})`}`),
		`user_auth.json present: ${report.userAuthPresent}`,
		`Manual wps_sid: ${report.wpsSid} (storage=${report.wpsSidStorage}${report.wpsSidProblem === void 0 ? "" : `, problem=${report.wpsSidProblem}`})`,
...
async function status(jsonOutput) {
	const authStatus = await makeStore().status();
	if (authStatus.state !== "signed-in") {
		...
		else process.stdout.write(`WPS Comate Connect: signed out\n`);
		return 1;
	}
	if (jsonOutput) {
		printJson({
...
async function check() {
	...
	if (outcome.ok) {
		process.stdout.write(outcome.content === true ? `OK: credential accepted, stream completed, content received (model=${model.id})\n` : `OK: credential accepted, stream completed, but no text arrived (model=${model.id}; the probe caps output at 8 tokens${outcome.reasoning === true ? ", reasoning only" : ""})\n`);
		return 0;
	}
	if (outcome.accepted === true) {
		process.stdout.write(`INCOMPLETE: the credential was accepted, but the probe did not finish — HTTP ${outcome.status} [${outcome.kind}]: ${(outcome.message ?? "").slice(0, 300)}\n`);
		return 1;
	}
...
*/
async function seal(jsonOutput) {
	const store = makeStore();
	const keyFile = store.keyFile();
	let raw = (process.env["WPS_COMATE_SID"] ?? "").trim();
	if (raw === "") {
		if (process.stdin.isTTY === true) process.stderr.write("Paste the wps_sid value, then press Enter (Ctrl+D / Ctrl+Z to cancel):\n");
		raw = await readStdin();
	}
	const stripped = raw.replace(/^wps_sid=/iu, "").trim();
	if (stripped !== raw.trim()) process.stderr.write("dsh-connect-comate: stripped the \"wps_sid=\" prefix from the pasted cookie\n");
	if (stripped === "") {
...
async function run(argv) {
	...
	if (flags.filter((flag) => flag !== "--json").length > 0 || jsonOutput && action !== "doctor" && action !== "status" && action !== "seal") {
	...
	try {
		switch (action) {
			...
			case "seal": return await seal(jsonOutput);
			case "status": return await status(jsonOutput);
			case "logout":
				await makeStore().logout();
				process.stdout.write(`WPS Comate Connect: v0.1 keeps no plugin-owned credential copy; the Comate desktop files were never touched.\n`);
				return 0;
		}
	} catch (error) {
