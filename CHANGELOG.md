...
> 每条聊天都走的路，脱敏原来只装在按按钮的探测上），以及**「测试连接」只看 HTTP 状态码、
> 于是假成功**（`0.4.2-rc.3`，空正文 200 / 流内 `error` 的 200 都被念成「连接成功」）。
> 中间夹一次 rc.1 自己引入的**过度脱敏**修正：网关真实的符号错误码 `not_login` 被抹掉、
> 让「未登录」这条最常见报错失去可搜索标记（`0.4.2-rc.2`）。
> 0.4.2 = 0.4.1 之后的**全部**改动。已打 tag `v0.4.2` 并推 origin；
> 安装：`github:DiEr-TouShou/dsh-connect-comate#v0.4.2`。未上 npm。
> 每条改动的完整推理与验收记录在下面三条 rc 条目里，这里只列入口。

### Bug Fixes

- **聊天错误原文不脱敏**（`0.4.2-rc.1`）：上游网关的错误正文里可能回显请求头、回显 `Cookie`、
  或把请求 URL 抄回来，之前这些字节被原样写进下游错误 JSON——等于把真实凭据交给 pi-ai 与
  浏览器面板。修法不是给三个调用点各加一行，而是把**唯一出口**收口：`writeOpenAIError`
  内部统一过 `safeMessage`（低频的 `check.ts` 有这层、高频的 `shim.ts` 没有，修的就是这）。
  顺带补齐脱敏规则本身：旧规则只认 JWT、`key=value`、`wps_sid=` 三种形状，`Bearer sk-live-…`
  与 JSON 的 `{"token":"…"}` 原样通过；规则搬到零依赖的 `src/redact.ts`，三处共用一份，
  并让**截断发生在脱敏之后**（跨截断点的半个真凭据也是凭据）。
- **`code` 例外太窄，把网关真实的错误码也抹了**（`0.4.2-rc.2`）：rc.1 只放行「纯数字 ≤ 6 位」，
  而网关的未登录响应实际是**符号标识** `code: "not_login"`——它被抹成 `[redacted]` 比修复前
  倒退，还正是用户要去搜、`classifyUpstreamError` 自己用来分类的标记。例外改为放行标识符形状
  （字母开头、字母数字/下划线、≤ 32 字符），凭据形状的值（`-` `.` `+` `/` `=` 全不在允许集）
  照旧过不去。
- **「测试连接」假成功 + 没有总超时**（`0.4.2-rc.3`）：`src/check.ts` 拿到 2xx 就 `cancel()`
  body 直接返回 `ok: true`，**body 一个字节都没读**——空正文的 200 与流内带 `error` 的 200
  （正是网关未登录的形状）都报成功。结论改为上游按顺序给出的三个事实：`accepted` / `completed`
  / `content`，`ok = accepted ∧ completed`；`content` **只报不判**（思考型模型全花在思考上、
  一个文本字不出是合法的通过）。另补总超时（15s，请求阶段与读取阶段都跟它赛跑，调用方已
  abort 的 signal 也能认出来）——上游只回状态行然后静默时，探测原本悬着不返回。

### 验收（合计）

- 单测从 377 增至 **395 测试绿**：`redact.spec.ts`（20 例规则矩阵）、`shim-redaction.spec.ts`
  （5 例真 shim 真 HTTP 回环钉出口字节）、`check.spec.ts`（13 → 30 例，同一形状不同结论的对照）。
- 三个真机验证脚本全部新增反向对照：`verify:redaction` 3 → 4 条（打 rc.1 产物 3/4 FAIL，
  只挂新增的第 4 条）、`verify:probe` 7 条（打 rc.2 产物 1/7，六条红——空正文 / 流内 error /
  静默 200 全被判成功，且缺总超时由看门狗 FAIL 证明）、`verify:card` 70 → 74 条断言。
- 两条 `tsc` 配置无错。`verify:shim` 真凭据链路照旧通过。

## 0.4.2-rc.3 (2026-09-26)

> 修掉「测试连接」的**假成功**：它只看 HTTP 状态码，于是空正文的 200、流内带 `error` 的
> 200 都被念成「连接成功」。顺带补上它一直缺的总超时。

### Bug Fixes

- **探测只看 HTTP 状态码，于是假成功**（真机复现）：`src/check.ts` 拿到 2xx 就 `cancel()`
  body 直接返回 `ok: true`，而 `ok` 的唯一来源是 `upstream.ts` 的 `response.ok`——**body
  一个字节都没读**。两种真实形状因此都报成功：

  - 空正文的 HTTP 200；
  - 流内带 `error` 事件的 HTTP 200——**这正是网关未登录的形状**
    （`{"error":{"code":"not_login"}}`，见 0.4.2-rc.2）。

  结论改为上游按顺序给出的**三个事实**，不再是单一布尔：`accepted`（HTTP 200，且流里
  没有任何事件说会话失效）、`completed`（流自己结束、且无流内 error）、`content`
  （至少一个非空文本 delta）。`ok = accepted ∧ completed`。

  `content` **只报不判**：探测把输出压在 8 个 token，思考型模型完全可能全花在思考上、
  一个文本字都不出——把这判成「连接坏了」是错的。所以「往返干净但没文本」是**通过**，
  只是措辞要如实说出来，不能像原来那样含糊地念一句「连接成功」。

- **没有总超时**（真机复现）：`ComateCheckOptions.signal` 的两个调用方都没传，上游只回
  状态行然后静默、或连状态行都不回时，探测**悬着不返回**。现在有总超时
  （`COMATE_CHECK_TIMEOUT_MS` = 15s），**请求阶段与读取阶段都跟它赛跑**——`abort` 是释放
  socket 的手段，不是让探测返回的手段；读取另设上限（`COMATE_CHECK_READ_LIMIT`）。
  调用方传进来的 signal 若**已经**是 aborted 状态现在会被认出来（`abort` 事件不会补发，
  旧写法在这一点上会一直等到超时）。

- **措辞**：卡片的 `row.testOk`（「连接成功。测试所用模型：{model}」）拆成三句——
  **有文本 / 无文本 / 凭据已通过但探测没跑完**；CLI 同理（`OK: … content received` /
  `OK: … but no text arrived` / `INCOMPLETE: …`）。

  「凭据已通过」与「探测失败」分开是有意的：上游可能**收下凭据再在流里报错**（积分不足、
  会话失效），也可能收下凭据什么都不回。这时该看的是上游那句话，而不是被一句笼统的失败
  推去重填 sid。`ComateCheckOutcome` 的三个新字段全是可选的——旧的 `{ok, model}` 形状
  仍然能解析，只是读不出分段事实。

### 验收

- `tests/check.spec.ts` 从 13 个用例增至 **30 个**（全量 **378 → 395 测试绿**）。新增的都是
  「同一形状、不同结论」的对照：空正文 / 流内 error / 只回状态行 / 有事件但流不结束 /
  请求阶段超时 / 调用方预先 abort / `content: false` 仍算通过 / reasoning-only。
- 新增 `pnpm run verify:probe`：**七个用例**打构建产物（本地桩当上游），断言 `ok` **和**
  accepted/completed/content 三个分段事实——只判 `ok` 会漏掉「凭据通过但探测没跑完」这一档。
  **不需要登录态**：假成功是判断错误，不该只在有账号的机器上才被检查。

  反向对照（同一脚本打 `0.4.2-rc.2` 的已安装产物）：**1/7**，六条红。其中五条的实际结果
  是 `{"ok":true,"model":"model-a"}`——空正文、流内 error、静默的 200 全被判成成功；
  第六条「上游根本不应答」由看门狗判 FAIL（4000ms 不返回），证明缺总超时。
- `verify:card` 从 70 增至 **74 条断言**：新增第 20 组「测试连接」，把四句话逐句钉住
  （成功 / 通过但没文本 / 凭据通过但没跑完 / 上游拒绝），并让探测路由的答复可替换。
- 其余不变：`verify:redaction` 4/4、`verify:card` 74/74、`verify:probe` 7/7，两条 `tsc` 配置无错。

## 0.4.2-rc.2 (2026-09-26)

> 修正 `0.4.2-rc.1` 自己引入的一处**过度脱敏**：错误码被当成秘密抹掉了。

### Bug Fixes

- **`code` 例外太窄，把网关真实的错误码也抹了**（真机复现）：rc.1 只放行「纯数字 ≤ 6 位」，
  而网关的未登录响应实际是**符号标识**：

  ```json
  {"error":{"message":"未登录，请先登录","type":"authentication_error","code":"not_login"}}
  ```

  `not_login` 会被抹成 `[redacted]`。这比修复前**倒退**——旧规则只匹配 `code=`，
  不碰 JSON 的 `"code":"…"`，而 `not_login` 正是用户要去搜、`classifyUpstreamError`
  自己也用来分类的标记。它是过度脱敏，不是安全漏洞，但会让「未登录」这条最常见的报错
  失去可搜索的标记。

  例外改为放行**标识符形状**：纯数字 ≤ 6 位，或字母开头、字母数字/下划线、≤ 32 字符。
  凭据形状的值照样过不去——`-` `.` `+` `/` `=` 全不在允许集里，base64、UUID、JWT、
  `sk-live-…` 都被更早的规则先一步抹掉，`code` 不会成为藏凭据的后门。

  残留缺口（自觉的，写进了测试）：一个纯字母数字、≤ 32 字符的裸令牌恰好出现在 `code`
  下会漏。二选一里选了「保留错误码」——它被抹掉是每次会话失效都会发生的伤害，
  而 `code` 里出现裸令牌是不现实的形状。

### 验收

- `tests/redact.spec.ts` 的 `code` 用例从 2 个改为 3 个（两种真实形状 / 凭据形状仍被抹 /
  明文记录残留缺口），总数 377 → **378 测试绿**。
- `pnpm run verify:redaction` 从 3 条用例增至 **4 条**，新增
  `upstream error with a symbolic code`；计数改为动态，输出 `PASS 4/4`。

  反向对照（同一个脚本打 `0.4.2-rc.1` 的已安装产物）：**3/4 FAIL**，且只挂新增的第 4 条
  ——原 3 条通过证明 rc.1 的防泄漏修复确实生效，第 4 条失败证明这条诊断回归是真实且独立的。
...
- **脱敏规则本身覆盖不全**：旧规则只认 JWT、`key=value`、`wps_sid=` 三种形状，
  ...
  - `code` 是唯一带例外的键：网关把会话失效写成 `code=12153`，那个数字是用户要去搜的、
    ...
    `code=<非数字>` 照旧当秘密。**（此条已被 0.4.2-rc.2 修正：符号错误码 `not_login`
    也是网关的真实形状，详见上方条目）**
...
> **给模型改显示名**（别名）与**手动放出 Off / Xhigh / Max 三档思考强度**（`0.4.1-rc.4`）；
> 另加一次纯 dev 修复：真机验收脚本不再把本机用户名写进源码（`0.4.1-rc.2`）。
> 0.4.1 = 0.4.0 之后的**全部**改动。已打 tag `v0.4.1` 并推 origin；
> 安装：`github:DiEr-TouShou/dsh-connect-comate#v0.4.1`。未上 npm。
> （`rc.3` 从未发布过，编号跳过。）
> 每条改动的完整推理与验收记录在下面三条 rc 条目里，这里只列入口。

### Features

- **每个模型可以单独设输出上限**（`0.4.1-rc.1`）：卡片「启用的模型」里每个模型右侧一个输入框。
  优先级四层：`WPS_COMATE_MAX_TOKENS`（env）> `maxOutputTokensByModel[modelId]`（这个模型自己的条目）
  > `maxOutputTokens`（全局默认）> 插件默认 32000。保存即生效，无需重启。

  **「留空」与「填 0」是两件事**：留空 = 这个模型**没有条目**、跟随全局默认值；填 `0` = 这个模型
  **不设上限**（出站不带 `max_tokens` 字段）。所以撤销一条覆盖就是清空那个框，界面里不需要第二个
  「复位」控件。逐模型解析落在 harness 唯一会读的那个字段上，不设上限的模型**整条缺席**而不是记成
  `0`——这一层的 `0` 会被原样物化成 `max_tokens: 0`，而 pi-ai 在 `defaultMaxTokens` 上要的是正整数。

- **模型别名：给模型改一个自己认得出的显示名**（`0.4.1-rc.4`）：卡片里每个模型多一个输入框，
  空着就用上游发现的名字（框里的灰字就是那个名字）。

  **别名只改画什么，不改 id。** DSH 选择器画的是描述符的 `name`，而请求、设置文档、逐模型上限表
  用的都是 id——所以别名只在 `name` 上落地（`name: alias ?? info.name`）。给模型改个名不可能弄丢
  `maxOutputTokensByModel` 里那条上限，也不可能让 `agent-default-model` 里存的选择失效。解析规则在
  零依赖的 `src/model-alias.ts`，浏览器与宿主 import **同一份**代码（避免两边 trim 规则分叉）。

- **手动放出 Off / Xhigh / Max 三档思考强度**（`0.4.1-rc.4`）：卡片新增「思考档位（高级）」一行，
  三个勾选框，**默认全不勾**；不勾时选择器里还是原来的四档（minimal / low / medium / high）。

  默认关是因为每一档都是插件不能替用户拍的板：

  - **Off 会连带改掉「不点名档位」的含义。** `dsh-llm-pi-ai` 把 `off` 折成「不传该选项」，而选择器
    自带的「provider default」走的也是同一条不发参数的路。两个不同的档位共享同一条出站形状，所以
    要让 Off 是真的 Off，只能给 `thinkingLevelMap.off` 一个**具名**线值：代价是「provider default」
    也跟着变成不思考——这不是 bug，是这条链路上「关闭」与「默认」不可兼得。
  - **Xhigh / Max 上游接受，但没测出与 high 的区别**（见下面「实测证据」）。

### Bug Fixes

- **真机脚本把本机用户名写进了源码**（`0.4.1-rc.2`）：`C:/Users/ASUS/.dsh/profiles/desktop/...`
  在仓库里躺了三份。CI、同事的机器、另开一个 Windows 账户都会在这里挂，而挂法是
  `ENOENT: C:\Users\ASUS\...`——看起来像插件坏了，其实只是脚本认路认死了。现在路径解析只有一份
  （`scripts/live-profile.mjs`，**零用户名字面量**），全不命中时把试过的每个路径都印出来。
  同一族还有个 `USERPROFILE` 只有 Windows 有的问题（别的平台上会拼出一个相对 cwd 的假路径）。
- **`verify:card` 的 jsdom 环境里 React 一直跑在「无 DOM」模式**（`0.4.1-rc.1`，脚本自身的 bug，
  不影响插件产物）：react-dom 在**模块加载那一刻**就用 `window` 判断自己在不在浏览器里，而脚本是先
  `require('react-dom')`、后铺全局——于是它挑了老 IE 那套监听而不是 `input`，**往输入框里敲字没人接**。
  这个 bug 只有在新断言要求「敲字」时才暴露：修复前 39 条里 10 条红，且全红在这一处。

### 实测证据：`off` 才是关思考的开关

10 个模型、裸打上游（绕开适配器）的原始采集结果：`reasoning_effort: off` **7/10 归零**，
而 OpenAI 词汇表里的 `none` **0/10 归零**（接受、但一律被忽略）。所以 `off` 的正式线值是 `off`，
**不是** `none`——这一条是真机测出来的，不是照抄词汇表。

两个例外都不是「开关无效」：`glm-5.3` / `glm-5.3-flash` 收到 `off` 后思考**反而更长**
（810→3059、660→1357）；`kimi-k3` 的基线本身就是 0，不能当生效证据。`xhigh` / `max` 上游接受
（HTTP 200、无报错），但**同一档位两次之间的差距比档位之间的差距还大**——测不出与 `high` 的区别，
所以它们是「可选但未经证明」的档位，按明确要求放出来，而不是当默认。

两张原始表（逐模型字符数）见 `0.4.1-rc.4`。

### Tests

- 默认套件 **257 → 351 例**（另 8 例真机用例默认跳过，分布 4 个文件）。新增文件：
  `model-alias.spec.ts`（21）、`thinking-levels.spec.ts`（25）、`thinking-levels-live.spec.ts`（3，真机）、
  `thinking-probe-live.spec.ts`（上面两张表的原始采集器，只产证据不做断言）。
- `max-tokens.spec.ts` 23 → 37、`settings-write.spec.ts` 31 → 62（别名表与档位表的写回与校验）。
- `verify:card`（jsdom 里跑**真渲染产物**）从 12 节 / 39 条扩到 19 节 / **70 条断言**。
...
| `thinking-probe-live` | 真网关（证据采集器） | 两张表的数字出自它 |
...
安装产物的 `lib/` 与源码树构建结果逐字节一致（6/6 md5），但上面的表并不因此就重复：产物验收
验的是**接线**（设置 → 描述符 → 适配器），那是构建不会帮你验的部分。最后一行用一份临时 vitest
配置把 spec 里的 `../src/*.ts` 全部改指到产物的 `lib/index.js`，于是**同一份验收 spec 跑的是产物**；
那份配置没进版本库（一次性）。

### Packaging

- 版本 0.4.0 → **0.4.1**（同日三条候选：rc.1 → rc.2 → rc.4，rc.3 跳过）。
- README 的固定版本示例更新为 `#v0.4.1`。

## 0.4.1-rc.4 (2026-09-26)

> 两件「默认不动、用户自己按需打开」的能力：**给模型改显示名**（别名），以及
> **手动放出 Off / Xhigh / Max 三档思考强度**。
> 已打 tag `v0.4.1-rc.4`；安装：`github:DiEr-TouShou/dsh-connect-comate#v0.4.1-rc.4`。未上 npm。
> （rc.3 从未发布过，编号跳过；上一次发布是 rc.2。）
>
> 默认行为一字不变：不填别名 = 用上游发现的名字，不勾档位 = 选择器里还是原来的四档。

### Features

- **模型别名：给模型改一个自己认得出的显示名。** 卡片「启用的模型」里每个模型多一个输入框，
  空着就是上游发现的名字（框里的灰字就是那个名字），填了就以你填的为准。

  **别名只改画什么，不改 id。** DSH 选择器画的是描述符的 `name`，而请求、设置文档、逐模型上限表
  用的都是 id——所以别名只在 `name` 上落地（`name: alias ?? info.name`），id 一字不动。给模型改个名
  不可能弄丢 `maxOutputTokensByModel` 里那条上限，也不可能让 `agent-default-model` 里存的选择失效。
  这条由 `verify:card` 第 13 节按「画的名字变了、发给宿主的还是那个 id」钉住。

  解析规则（`src/model-alias.ts`，零依赖、浏览器与宿主**同一份**代码）：键和值都 trim；trim 后为空的
  键或值都不进表——空串不是「把名字改空」而是「没有别名」，这也是清空输入框能撤销别名的原因；
  值不是字符串的条目只丢自己，其余照用。用 `Map` 而不是普通对象：键来自上游给的模型 id，
  `__proto__` 这种键不该有机会碰到原型。撤销别名 = 清空那一格，整张表一次性重写，设置文档里不留空串。

  保存时的校验不能拿原始文本比对：同一张表换个键序重建就会读成「变了」，而那种「保存失败」
  用户根本没法处理。所以指纹用排序后的 `JSON.stringify`（`aliasKey`）——键序无关，且转义安全：
  别名是用户敲的任意文本，拼字符串当指纹的话 `{"a": "x\ny=z"}` 会和 `{"a": "x", "y": "z"}` 撞上，
  一个多行别名就能撞。

- **手动放出 Off / Xhigh / Max 三档思考强度。** 卡片新增「思考档位（高级）」一行，三个勾选框，
  **默认全不勾**；勾了的档位才会追加到模型选择器里（选择器原有的四档 minimal / low / medium / high
  一直都在）。保存即生效，无需重启。

  **这三档默认关闭，是因为每一个都是插件自己不能替用户拍的板：**

  - **Off 会连带改掉「不点名档位」的含义。** `dsh-llm-pi-ai` 把 `off` 改写成「不传该选项」
    （`profileOptions()`：`reasoning === 'off' ? undefined : reasoning`），而选择器自带的
    「provider default」走的也是同一条不发参数的路。两个不同的档位共享同一条出站形状，
    所以要让 Off 是真的 Off，就只能给 `thinkingLevelMap` 的 `off` 键一个**具名**线值：
    `off: 'off'`。代价是「provider default」也跟着变成不思考——这不是 bug，是这条链路上
    「关闭」与「默认」不可兼得。不勾这一档时 `off: null`，两个位置都不出现，一切照旧。
  - **Xhigh / Max 上游接受，但没测出与 high 的区别。** 见表二。

  `off` 的正式线值就是 `off`，**不是** OpenAI 词汇表里的 `none`——见下面「实测证据」与
  「修正」两节，这一条是真机测出来的，不是照抄词汇表。

### 实测证据（2026-09-26，本机，目录里 10 个模型，裸打上游、绕开适配器）

表一：`off` 与 `none` 对同一道题（提示词要求思考）的 `reasoning_content` 字符数。

| 模型 | 基线（不发档位） | `reasoning_effort: off` | `reasoning_effort: none` |
| --- | ---: | ---: | ---: |
| `deepseek/deepseek-v4-flash/` | 656 | **0** | 373 |
| `deepseek/deepseek-v4-pro/` | 398 | **0** | 839 |
| `deepseek/deepseek-v4.1-flash/` | 313 | **0** | 205 |
| `minimax/MiniMax-M3/` | 104 | **0** | 1163 |
| `moonshot/kimi-k3/` | 0 | 0 | 273 |
| `xiaomi/mimo-v2.5-pro/` | 132 | **0** | 201 |
| `xiaomi/mimo-v2.5/` | 1609 | **0** | 1207 |
| `zhipu/glm-5.2/` | 1242 | **0** | 1665 |
| `zhipu/glm-5.3-flash/` | 660 | 1357 | 553 |
| `zhipu/glm-5.3/` | 810 | 3059 | 776 |

结论：**`off` 才是关思考的开关（7/10 归零）；`none` 会被接受却一律被忽略（0/10 归零）。**
两个例外都不是「开关无效」：`glm-5.3` / `glm-5.3-flash` 收到 `off` 后思考反而更长（810→3059、
660→1357），像是把不认识的取值当成了「放开想」；`kimi-k3` 的基线本身就是 0（同一道题上 `none`
却回了 273 字符），它在这里不能当 `off` 生效的证据，只能算「没有反例」。

表二：`high` / `xhigh` / `max` 各跑两次（同一道题，单次样本分不清档位差异和采样噪声）。

| 模型 | high | xhigh | max |
| --- | --- | --- | --- |
| `deepseek/deepseek-v4-flash/` | 412, 365 | 332, 377 | 230, 335 |
| `deepseek/deepseek-v4-pro/` | 536, 722 | 628, 402 | 522, 392 |
| `deepseek/deepseek-v4.1-flash/` | 261, 244 | 236, 185 | 111, 157 |

结论：**测不出 `xhigh` / `max` 与 `high` 的区别**——同一档位两次之间的差距（`-pro` 的 high 是
536 与 722，差 186）比档位之间的差距还大。上游接受这两个值（HTTP 200、无报错），所以它们是
「可选但未经证明」的档位，按用户的明确要求放出来，而不是当默认。

### 修正（旧结论被这次复测推翻）

- **「三个 glm 都忽略 `off`」只对了两个。** 早先那次只测了 `glm-5.3` 与 `glm-5.3-flash`，
  这次把 `glm-5.2` 也放进去，它收到 `off` 后**照常归零**（1242→0）。按旧结论写文案会把
  「glm 家族整体忽略 off」这种没测过的话说出口，所以代码注释与 README 都按表一重写。
- **README 里「关闭思考当前做不到」那句，机制描述是对的，结论错了。** 原文说 `off` 永远到不了
  pi-ai、因此不提供这个选项——中间层改写 `off` 确实存在，但 `thinkingLevelMap` 的 `off` 键本来
  就是用来给这个档位一个具名线值的（`off: 'off'`），所以它到得了。上一版把一个可以绕过的中间层
  当成了硬墙，于是既没提供开关、也没说清代价；现在两样都有。
- **`kimi-k3` 不能当 `off` 生效的证据**（基线已是 0）。旧表格把它记成「6/10 归零」里的一员，
  这次单独标注。

### Tests

- 新增 `tests/model-alias.spec.ts` 21 例：解析规则（trim、空键/空值、非字符串条目、`__proto__` 键、
  原型不被污染）、指纹的键序无关性与转义安全性（含「多行别名撞指纹」的反例）、宿主按 id 取值。
- 新增 `tests/thinking-levels.spec.ts` 25 例：`buildThinkingLevelMap` 的形状（基础四档照原样、
  勾了的填线值、没勾的显式钉 `null` 而不是留空——pi-ai 对 `xhigh` / `max` 的缺省判定与基础档位相反）、
  解析容错（非数组读作「什么都没开」、认不出的档位丢掉、去重、顺序归一化）、以及**选择器顺序由
  pi-ai 决定**（`off` 在最前，不是卡片上的 off / xhigh / max 顺序——这条一开始写反了，真机才暴露）、
  出站 body 的档位形状、`off: null` 时字段不出现。
- 新增 `tests/thinking-levels-live.spec.ts` 3 例（真机，默认跳过）：开 `off` 后**同一个请求形状**
  出站真的带上了 `reasoning_effort=off` 且 `reasoning_content` 归零；`xhigh` / `max` 上游接受且仍在
  思考、`glm-5.3` 即使收到 `off` 也照常思考；`none` 与 `off` 在同一个模型上分道扬镳（前者照常思考）。
- 新增 `tests/thinking-probe-live.spec.ts`：上面两张表的**原始采集器**（绕开适配器直打上游，
  只打印观察结果）。与验收 spec 分工不同——一个验「插件现在是对的」，一个产证据。
- `tests/settings-write.spec.ts` 增加 22 例：别名表与档位表的写回与校验（整张表重写、空表 = 全清、
  回来不一样时报 `not-persisted`、键序/集合序无关、patch 省略时不动已存的值、不把调用方自己的对象
  交给宿主）。
- `scripts/verify-card-selection.mjs`（`verify:card`）从 12 节扩到 19 节、39 → **70 条断言**：
  别名框的个数/占位符/改名后当场重画/只写改过的那一个、清空即撤销（整张表重写）、三个档位框默认全不勾、
  勾了保存退出再进来仍在、外部改动只重新播种「没被碰过」的草稿、撤销修改把别名与档位一起还原。
- 默认套件 288 → **351 例**（另 8 例真机用例默认跳过）。
