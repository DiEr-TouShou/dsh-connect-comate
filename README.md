...
# 脱敏边界：起一个真 shim，喂它「上游错误正文里回显了凭据」的响应，断言出口字节里
# 一个字符都不剩，同时分类与原因还在。**不需要登录态**（上游是本地桩）——脱敏是安全
# 边界，不该只在有账号的机器上才被检查。默认 import 已安装产物，同 verify:sid-cipher
...
- **越过边界的第三方文本先脱敏**：上游错误正文、凭据解析异常、handler 内部异常，都在
  ...
  `code=12153` 这类纯数字错误码保留（它是用户排查时要搜的标记），
